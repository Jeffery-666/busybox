...
IF_FEATURE_ROTATE_LOGFILE( \
	...
	/* max size of file in flash before rotation */  \
	unsigned logFlashFileSize;              \
...
static const struct init_globals init_data = {
	...
	.logFlashFile = {
#ifdef INHAND_DG7
		.path = "/var/app/data/messages",//licj:change path to /var/app/log/messages in DG7
#else
		.path = "/var/backups/messages",
#endif
		.fd = -1,
	},
#ifdef SYSLOGD_MARK
	...
	.logFlashFileSize = 200 * 1024,
#ifdef INHAND_DG7
	.logFileRotate = 10,//licj:change to 10 in DG7
#else
	.logFileRotate = 1,
#endif
#endif
...
static TERM_INFO logterminal[MAX_TERMINALS] = {0};
...
enum {
	...
	IF_FEATURE_ROTATE_LOGFILE(OPTBIT_flashfilesize   ,) // -z
	...
	OPT_console     = 1 << OPTBIT_console ,
	OPT_flash       = 1 << OPTBIT_flash   ,
	OPT_timestamp   = 1 << OPTBIT_timestamp,
	OPT_filesize    = IF_FEATURE_ROTATE_LOGFILE((1 << OPTBIT_filesize   )) + 0,
	OPT_flashfilesize    = IF_FEATURE_ROTATE_LOGFILE((1 << OPTBIT_flashfilesize   )) + 0,
...
#define OPTION_STR "m:nO:l:ScF:t" \
...
#define OPTION_DECL *opt_m, *opt_l, *opt_F \
	IF_FEATURE_ROTATE_LOGFILE(,*opt_s) \
	IF_FEATURE_ROTATE_LOGFILE(,*opt_z) \
	IF_FEATURE_ROTATE_LOGFILE(,*opt_b) \
	IF_FEATURE_IPC_SYSLOG(    ,*opt_C = NULL) \
	IF_FEATURE_SYSLOGD_CFG(   ,*opt_f = NULL)
#define OPTION_PARAM &opt_m, &(G.logFile.path), &opt_l, &opt_F \
	IF_FEATURE_ROTATE_LOGFILE(,&opt_s) \
	IF_FEATURE_ROTATE_LOGFILE(,&opt_z) \
	IF_FEATURE_ROTATE_LOGFILE(,&opt_b) \
 ...
 reopen:
	...
	if (G.logFileSize && log_file->isRegular && log_file->size > G.logFileSize) {
		if (G.logFileRotate) { /* always 0..99 */
			...
			char newFile[i];
#ifdef INHAND_DG7
			i = 0;//licj:force limit to 1 in DG7
#else
			i = G.logFileRotate - 1;
#endif
...
/*begin of zly*/
/* Print a message to the log file in flash. */
static void log_flash(time_t now, char *msg, logFile_t *log_file)
{
#ifdef SYSLOGD_WRLOCK
	struct flock fl;
#endif
...
reopen:
...
#if ENABLE_FEATURE_ROTATE_LOGFILE
	if (retry<2 && G.logFlashFileSize && log_file->isRegular && log_file->size > G.logFlashFileSize) {
		if (G.logFileRotate) { /* always 0..99 */
			int i = strlen(log_file->path) + 3 + 1;
			char oldFile[i];
			char newFile[i];
			...
			/* newFile == "f.0" now */
			ret = rename(log_file->path, newFile);
#ifdef SYSLOGD_WRLOCK
			fl.l_type = F_UNLCK;
			fcntl(log_file->fd, F_SETLKW, &fl);
#endif
			close(log_file->fd);
			/*zly: rename fail because jffs2 problem*/
			if(ret<0) {
				unlink(newFile);
				unlink(log_file->path);
			}
	...
	/*zly: close flash file every time*/
	close(log_file->fd);
	log_file->fd = -1;
}

/* Print a message to /dev/console. */
static void log_console(char *msg)
{
	if (logconsoleFD<0) {
		logconsoleFD = device_open(_PATH_CONSOLE, O_WRONLY | O_NOCTTY | O_NONBLOCK);
	}

	if(logconsoleFD>=0){
		full_write(logconsoleFD, msg, strlen(msg));
	}
}
/*end of zly*/

/* begin: wucl added terminal monitor support */
#define TERM_LOG_CONFIG_FILE "/etc/term_log.conf"

static void log_terminal_fd_init(void)
{
	int i = 0;

	for(i = 0; i < MAX_TERMINALS; i++){
		bzero(&logterminal[i], sizeof(TERM_INFO));
		logterminal[i].fd = -1;
	}
...
static void log_terminal_fd_add(char *term)
{
	int free_idx = -1;
	int i = 0;

	for(i = 0; i < MAX_TERMINALS; i++){
		if(!strcmp(logterminal[i].name, term)){
			logterminal[i].fd = device_open(term, O_WRONLY | O_NOCTTY | O_NONBLOCK);
			logterminal[i].refcount = 2;
			//syslog(LOG_INFO, "log_terminal_fd_add update: i:%d, fd: %d\n", i, logterminal[i].fd);
			return;
		}

		if(free_idx == -1 && logterminal[i].fd < 0){
			free_idx = i;
		}
	}

	if(free_idx != -1){
		logterminal[free_idx].fd = device_open(term, O_WRONLY | O_NOCTTY | O_NONBLOCK);
		//strlcpy(logterminal[free_idx].name, term, sizeof(logterminal[free_idx].name));
		snprintf(logterminal[free_idx].name, sizeof(logterminal[free_idx].name), "%s", term);
		logterminal[free_idx].refcount = 2;
		//syslog(LOG_INFO, "log_terminal_fd_add: i:%d, fd: %d\n", free_idx, logterminal[free_idx].fd);
	}

	return;
}

static void log_terminal_fd_refcnt_dec(void)
{
	int i = 0;

	for(i = 0; i < MAX_TERMINALS; i++){
		/* old ones to be removed */
		if(logterminal[i].refcount == 1){
			//syslog(LOG_INFO, "disable terminal monitor for: %s\n", logterminal[i].name);
			close(logterminal[i].fd);
			bzero(&logterminal[i], sizeof(TERM_INFO));
			logterminal[i].fd = -1;
		}

		/* keep newly added */
		if(logterminal[i].refcount >= 2){
			logterminal[i].refcount = 1;
		}
	}
...
/* wucl load terminal monitoer session at startup */
static void log_terminal_fd_load(void)
{
	char line[128] = {0};
	FILE *fp = NULL;

	fp = fopen(TERM_LOG_CONFIG_FILE, "r");
	if (!fp){
		//syslog(LOG_INFO, "fopen error.\n");
		return;
	}

	while (fgets(line, sizeof(line), fp)) {
		//syslog(LOG_INFO, "enable terminal monitor for: %s\n", line);
		trim(line);
		log_terminal_fd_add(line);

		bzero(line, sizeof(line));
	}

	log_terminal_fd_refcnt_dec();

	return;
}

/* wucl Print a message to telnet|ssh session. */
static void log_terminal(char *msg)
{
	int i = 0;

	for(i = 0; i < MAX_TERMINALS; i++){
		if(logterminal[i].fd >= 0){
			full_write(logterminal[i].fd, msg, strlen(msg));
		}
	}

	return;
}
/* end: wucl added terminal monitor support */
...
static void timestamp_and_log(int pri, char *msg, int len)
{
	...
	if (LOG_PRI(pri) < G.logLevel) {
		...
		/*zly: log important message to flash*/
		if ((option_mask32 & OPT_flash)
				&& (LOG_PRI(pri) < G.logFlashLevel)) {//err--3, warning--4, notice--5
			log_flash(now, G.printbuf, &G.logFlashFile);
		}    

		/*zly: log all message to console*/
		if (option_mask32 & OPT_console) {
			log_console(G.printbuf);
		}    

		/* wucl: terminal moniter for telnet/ssh */
		log_terminal(G.printbuf);
	}
}
...
static void update_termon_session(int sig) 
{
	log_terminal_fd_load();
}
...
#define recvbuf (G.recvbuf)
	...
	alarm(G.markInterval);
#endif
	/* wucl added terminal monitor support. */
	signal(SIGUSR1, update_termon_session);
	...
	timestamp_and_log_internal("syslogd started: v" BB_VER);

	/* wucl added terminal monitor support. */
	log_terminal_fd_init();
	log_terminal_fd_load();
...
int syslogd_main(int argc UNUSED_PARAM, char **argv)
{
	...
	//if (opts & OPT_small) // -S
	if (option_mask32 & OPT_flash) // -F
		G.logFlashLevel = xatou_range(opt_F, 1, 8);
#if ENABLE_FEATURE_ROTATE_LOGFILE
	if (opts & OPT_filesize) // -s
		G.logFileSize = xatou_range(opt_s, 0, INT_MAX/1024) * 1024;
	if (opts & OPT_flashfilesize) // -z
		G.logFlashFileSize = xatou_range(opt_z, 0, INT_MAX/1024) * 1024;
