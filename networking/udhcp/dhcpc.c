...
#if 0
	bb_error_msg("send client mac addr %02x:%02x:%02x:%02x:%02x:%02x",
			client_data.client_mac[0],client_data.client_mac[1],
			client_data.client_mac[2],client_data.client_mac[3],
			client_data.client_mac[4],client_data.client_mac[5]);
#endif
...
int udhcpc_main(int argc UNUSED_PARAM, char **argv)
{
	...
	int retval;
	const char *iface_mac;
	unsigned int client_mac[CLIENT_MAC_SIZE]={0};
	int i, input_mac_flag = 0;
	...
	opt = getopt32long(argv, "^"
		...
		"CV:H:h:F:m:i:np:qRr:s:T:+t:+SA:+O:*ox:*fB"
		USE_FOR_MMU("b")
	...
	if(strncmp(iface_mac, CLIENT_MAC_DEF,CLIENT_MAC_SIZE)){
		sscanf(iface_mac, "%02x-%02x-%02x-%02x-%02x-%02x", 
				&client_mac[0],&client_mac[1],&client_mac[2],
				&client_mac[3],&client_mac[4],&client_mac[5]);
		for(i = 0; i<CLIENT_MAC_SIZE; i++){
			client_data.client_mac[i] = client_mac[i]; 
		}    
		input_mac_flag = 1; 
	}
...
#if 0
	bb_error_msg("client mac addr %02x:%02x:%02x:%02x:%02x:%02x",
			client_data.client_mac[0],client_data.client_mac[1],
			client_data.client_mac[2],client_data.client_mac[3],
			client_data.client_mac[4],client_data.client_mac[5]);
	bb_error_msg("input client mac addr %02x:%02x:%02x:%02x:%02x:%02x",
...
#if 0
				bb_error_msg("client mac2 addr %02x:%02x:%02x:%02x:%02x:%02x",
						client_data.client_mac[0],client_data.client_mac[1],
						client_data.client_mac[2],client_data.client_mac[3],
						client_data.client_mac[4],client_data.client_mac[5]);
#endif
