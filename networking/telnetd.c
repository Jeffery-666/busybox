...
#if ENABLE_FEATURE_TELNETD_STANDALONE
	if (IS_INETD) {
		G.sessions = make_new_session(0, NULL);
...
#else
	G.sessions = make_new_session(NULL);
	if (!G.sessions) /* pty opening or vfork problem, exit */
		return 1; /* make_new_session printed error message */
#endif
 ...
 again:
	...
	if (!IS_INETD && FD_ISSET(master_fd, &rdfdset)) {
...
#else
		fd = accept(master_fd, &fromAddr.u.sa, &fromAddr.len);
#endif
		if (fd < 0)
			goto again;
		close_on_exec_on(fd);
		...
		if (fromAddr.u.sa.sa_family == AF_INET) {
			inet_ntop(AF_INET, &fromAddr.u.sin.sin_addr,
					buf, sizeof(buf));
		}   
#if ENABLE_FEATURE_IPV6
		if (fromAddr.u.sa.sa_family == AF_INET6) {
			inet_ntop(AF_INET6, &fromAddr.u.sin6.sin6_addr,
					buf, sizeof(buf));
		}   
#endif

		/* Create a new session and link it into active list */
		new_ts = make_new_session(fd, buf);
		if (new_ts) {
