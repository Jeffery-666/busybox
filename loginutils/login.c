...
enum {
	...
	/* support user name length up to 128 */
	USERNAME_SIZE = 131,
	TTYNAME_SIZE = 32,
};
...
void FAST_FUNC login_setup_environment(const char *shell, int flags, const struct passwd *pw);
...
static void alarm_handler(int sig UNUSED_PARAM)
{
	...
	sleep(1); /*add by daiyb for message print compelete*/
...
static void wait_active(void)
{
	...
	read(0,clearbuf,sizeof(clearbuf));
}
...
int login_main(int argc UNUSED_PARAM, char **argv)
{
	...
	/* add by daiyb */
	if (strcmp(short_tty,"ttyS0") == 0 || strcmp(short_tty,"console") == 0) {
		wait_active();
		//added by baodn
#if ENABLE_PAM
		console_entry = TRUE;
	...
	while (1) {
		...
		//added by baodn
		if (console_entry){
			pamret = pam_start("console", username, &conv, &pamh);
		}else{
			pamret = pam_start("telnet", username, &conv, &pamh);
		}
#endif
		if (pamret != PAM_SUCCESS) {
			failed_msg = "start";
		...
		pamret = pam_authorize(pamh, 0); 
		if (pamret != PAM_SUCCESS) {
			failed_msg = "pam_author";
			goto pam_auth_failed;
		}
		...
		if (!pw) {
			//addby baodn use a default user
			getpwnam_r("root", &pwdstruct, pwdbuf, sizeof(pwdbuf), &pw);
			if(!pw){
				goto auth_failed;
			}
		}
 ...
 auth_failed:
	...
	setup_environment(pw->pw_shell,
			...
			pw);
#else
	login_setup_environment(pw->pw_shell,
			(!(opt & LOGIN_OPT_p) * SETUP_ENV_CLEARENV) + SETUP_ENV_CHANGEENV,
			pw);
#endif
